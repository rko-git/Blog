...
class Content
{
    ...
    { //ziskanie kategorie podla id pouzivane pri mazani s GET
        try {
            $database = new Database();
            $db = $database->getConnection();
    ...
    public static function createPost(): bool
    {
        try {
            ...
            $obrazok = "sample.png";
            if (isset($_FILES["image"]) && $_FILES["image"]["error"] === UPLOAD_ERR_OK) {  //skontroluje ci bol nahrany obrazok a nenastala chyba
                $povolene = ["jpg", "jpeg", "png", "webp"];
                $pripona = strtolower(pathinfo($_FILES["image"]["name"], PATHINFO_EXTENSION)); //ziska priponu suboru
                if (!in_array($pripona, $povolene)) {
                    return false;
                }
                if ($_FILES["image"]["size"] > 8 * 1024 * 1024) { //8 MB limit obrazka
                    return false;
                }
